  ...
  return <>
    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-[1.35fr_.65fr]">
      ...
      <Panel title="Dynamic Call-to-Come" subtitle="Rule-controlled farmer release">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 text-sm"><Metric label="Next release" value={state.bottleneck ? "On hold" : "10:48 AM"}/><Metric label="Arrival window" value={arrivalWindow.split(" – ")[0]}/><Metric label="Farmers released" value="6"/><Metric label="Waiting at home" value="12"/></div>
      </Panel>
    </div>
...
function JudgeDemo({open,setOpen,auto,setAuto,goStep}:{open:boolean;setOpen:(v:boolean)=>void;auto:boolean;setAuto:(v:boolean)=>void;goStep:(n:number)=>void}) {
  const {state,dispatch}=useDemo(); const step=demoSteps[state.demoStep];
